 ...
 * @param {object} props
 * @param {string} props.d SVG path.
 * @param {number} props.delay Seconds before the loop starts.
 * @param {boolean} props.running Loop only while on screen.
 */
function SteamWisp({ d, delay, running }) {
  ...
  return (
    <m.path
      ...
      animate={
        reduce || !running
          ? { opacity: 0.4, y: 0, pathLength: 1 }
          : { opacity: [0.15, 0.85, 0.15], y: [2, -4, -7], pathLength: [0.3, 1, 0.35] }
      }
      transition={running ? { duration: 2.2, repeat: Infinity, delay, ease: 'easeInOut' } : { duration: 0.3 }}
    />
  );
}
 ...
 * The loops run only while the mascot is on screen.
 * @param {object} props
    ...
    >
      <svg viewBox="4 0 112 118" className="relative mx-auto h-auto w-full overflow-visible" aria-hidden="true">
        ...
        >
          ...
          <SteamWisp running={onScreen} delay={0.7} d="M74 34 C70 28 78 24 74 16" />
        </m.g>
      </svg>
    </m.div>
  );
